...
def load_graph(exp_dir: str, epoch: int, object_name: str) -> GraphObjectModel:
    if exp_dir.endswith("exp1_distant_train"):
        # Handle the older exp1_distant_train checkpoint structure where the pretrained model is nested under "pretrained/0/model.pt".
        model_path = Path(exp_dir) / "pretrained" / str(epoch) / "model.pt"
    else:
